(function($, window) {
	...
	proto.initEvent = function() {
		var self = this;
		$(document.body).on('tap', 'a[data-preview-src]', function() {
			self.open(this);
			return false;
		});
		var laterClose = null;
	...
	proto.addImages = function(group, index) {
		...
		if (group) {
			if (group === defaultGroupName) {
				imgs = document.querySelectorAll("a[data-preview-src]:not([data-preview-group])");
			} else {
				imgs = document.querySelectorAll("a[data-preview-src][data-preview-group='" + group + "']");
			}
		} else {
			imgs = document.querySelectorAll("a[data-preview-src]");
		}
		if (imgs.length) {
